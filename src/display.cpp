...
uuid::log::Logger Display::logger_{F_(display), uuid::log::Facility::DAEMON};
...
void Display::initialize()
{
  ...
  //shell.printfln(PSTR("[Display] Initialized"));
...
void Display::process(float temperature, float setPoint, float humidity, float relTemp)
{
    ...
    //shell.printfln(PSTR("[Display] Process %s <-> %s [%s] [%s]"), (char *)&temp_cur,  (char *)&temp_set, (char *)&hum_curr, (char *)&rel_temp);
    LOG_INFO(F("[Display] Process %s <-> %s [%s] [%s]"), (char *)&temp_cur,  (char *)&temp_set, (char *)&hum_curr, (char *)&rel_temp);
    display.clearDisplay();
